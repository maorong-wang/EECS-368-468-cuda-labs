...
//                h_data[i] = 1.0f;
    ...
    float* d_sum = NULL;
    float* d_cumsum = NULL;
    int numBlocks = 1 + (mem_size-1)/BLOCK_SIZE;
    CUDA_SAFE_CALL( cudaMalloc( (void**) &d_idata, mem_size));
    CUDA_SAFE_CALL( cudaMalloc( (void**) &d_odata, mem_size));
    CUDA_SAFE_CALL( cudaMalloc( (void**) &d_sum, numBlocks*sizeof(float)));
    CUDA_SAFE_CALL( cudaMalloc( (void**) &d_cumsum, numBlocks*sizeof(float)));
    ...
    // measurement
    prescanArray(d_odata, d_idata, d_sum, d_cumsum, 16);
    ...
    prescanArray(d_odata, d_idata, d_sum, d_cumsum, num_elements);
    // **===-----------------------------------------------------------===**
