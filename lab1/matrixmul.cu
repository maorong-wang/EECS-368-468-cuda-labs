...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
	...
	cudaMalloc((void **)&(d_M.elements),WM*HM*sizeof(float));
	cudaMemcpy(d_M.elements,M.elements,HM*WM*sizeof(float),cudaMemcpyHostToDevice);
	
	cudaMalloc((void **)&(d_N.elements),HN*WN*sizeof(float));
	cudaMemcpy(d_N.elements,N.elements,HN*WN*sizeof(float),cudaMemcpyHostToDevice);
	
	cudaMalloc((void **)&(d_P.elements),HP*WP*sizeof(float));

	dim3 dimGrid(HP,1,1);
	dim3 dimBlock(WP,1,1);
	MatrixMulKernel<<<dimGrid,dimBlock>>>(d_M,d_N,d_P);
	cudaMemcpy(P.elements,d_P.elements,HP*WP*sizeof(float),cudaMemcpyDeviceToHost);

	CUDA_SAFE_CALL(cudaFree(d_M.elements));
	CUDA_SAFE_CALL(cudaFree(d_N.elements));
	CUDA_SAFE_CALL(cudaFree(d_P.elements));
}
